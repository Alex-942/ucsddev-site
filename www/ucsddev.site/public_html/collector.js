...
/*fetchAll('https://ucsddev.site/api/static');
console.log("HERE");
console.log(keys);
for(var i = 0; i < keys.static.length; i++){
  if(keys.static[i].session_id == session_id){
    updateRecord(`https://ucsddev.site/api/static/${keys.static[i].id}`, static_data);
    break;
  }
  if(i == keys.static.length - 1){
...
/*function getCookie(cname) {
 ...
 session_id = getCookie('_lr_uf_-ezpkav');
console.log(static);
*/
...
//tracks the coords of the scrolls
//n cssCheck() {
//135   var css_test = document.createElement('div');
//136   css_test.style.position = 'absolute';
//137   document.getElementsByTagName('body')[0].appendChild(css_test);
//138   if (css_test.currentStyle)
//139  var currStyle = css_test.currentStyle['position'];
//140   else if (window.getComputedStyle)
//141  var currStyle = document.defaultView.getComputedStyle(css_test, null).getPropertyValue('position');
//142   cssEnabled = (currStyle == 'static') ? false : true;
//143   document.getElementsByTagName('body')[0].removeChild(css_test);
//144 }
//145
//Page scroll event
...
/*
setInterval( function activity_Record() {
